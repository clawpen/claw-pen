...
impl AuthManager {
    ...
    pub fn login(&self, password: &str) -> Result<TokenResponse, AuthError> {
        let stored_hash = self.admin_password_hash
            .as_ref()
        ...
        let refresh_token = self.generate_token("admin", "refresh", REFRESH_TOKEN_EXPIRATION_DAYS * 24 * 3600)?;
    ...
    pub fn refresh(&self, refresh_token: &str) -> Result<TokenResponse, AuthError> {
        ...
        // Generate new tokens
        let access_token = self.generate_token(&claims.sub, "access", JWT_EXPIRATION_HOURS * 3600)?;
        let new_refresh_token = self.generate_token(&claims.sub, "refresh", REFRESH_TOKEN_EXPIRATION_DAYS * 24 * 3600)?;
    ...
    fn generate_token(&self, subject: &str, token_type: &str, expires_in_seconds: i64) -> Result<String, AuthError> {
...
pub fn cli_set_password(data_dir: &PathBuf) -> Result<(), AuthError> {
    ...
    let password = stdin.lock().lines().next()
        .ok_or_else(|| AuthError::IoError(io::Error::new(io::ErrorKind::UnexpectedEof, "No input")))?
