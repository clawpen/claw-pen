...
impl NetworkBackend for TailscaleBackend {
    async fn assign_identity(&self, container_id: &str) -> Result<String> {
        ...
        Ok(format!("ts-{}", container_id[..8].to_string()))
    }
...
impl NetworkBackend for WireguardBackend {
    async fn assign_identity(&self, container_id: &str) -> Result<String> {
        ...
        Ok(format!("wg-{}", container_id[..8].to_string()))
    }
...
impl NetworkBackend for ZerotierBackend {
    async fn assign_identity(&self, container_id: &str) -> Result<String> {
        ...
        Ok(format!("zt-{}", container_id[..8].to_string()))
    }
...
// Factory function
pub fn create_backend(backend_type: &str, auth_key: Option<String>) -> Box<dyn NetworkBackend + Send + Sync> {
    match backend_type {
        "tailscale" => Box::new(TailscaleBackend { auth_key }),
        "wireguard" => Box::new(WireguardBackend),
        "zerotier" => Box::new(ZerotierBackend { network_id: String::new() }),
